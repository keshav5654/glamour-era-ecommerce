...
const CartModal = ({products,isOpen,onClose}) => {
  ...
  return (
    ...
    >
      <div className={`fixed right-0 top-0 md:w-1/3 w-full bg-white h-full overflow-y-auto transition:transfrom ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}
                        ...
                        <div key={index} className='flex flex-col md:flex-row md:items-center md:justify-between shadow-md md:p-5 p-2 mb-4'>
                            ...
                            <span className='mr-4 px-1 bg-primary text-white rounded-full'>
                                ...
                                <button 
                                    ...
                                    className='size-6 flex items-center px-1.5 rounded-full bg-gray-200 text-gray-700 hover:bg-primary hover:text-white ml-8'>-</button>
                                    <span className='px-2 text-center mx-1'>{item.quantity}</span>
                                    <button
                                    onClick={()=> handleQuantity('increment',item._id)} 
                                     className='size-6 flex items-center px-1.5 rounded-full bg-gray-200 text-gray-700 hover:bg-primary hover:text-white '>+</button>
                                     <div>
                                        <button
                                        onClick={(e) => handleRemove(e,item._id)}
                                         className='text-primary hover:text-red-800 mr-4 ml-4'>Remove</button>
                                    </div>
                                </div>
                            </div>
                        </div>
